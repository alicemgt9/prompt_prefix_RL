...
def load_tokenizer_only(model_size, hf_token=None):
    tokenizer = AutoTokenizer.from_pretrained(f'../../../GLM49B', use_auth_token=hf_token)
    return tokenizer

def load_unemb_only(model_size,model_version):
    if model_size == '7B' or model_size == '7b' and model_version == '2':
        lm_head_state_dict = load_file('../../../GLM49B/model-00004-of-00004.safetensors')
...
class MLPWrapper(torch.nn.Module):
    ...
    def forward(self, x):
        gate_up = self.gate_up_proj(x)                 # [B, L, 27392]

        gate, up = gate_up.chunk(2, dim=-1)           # each [B, L, 13696]

        post_activation = self.act_fn(gate) * up  # SwiGLU
...
class GLMHelper:
    def __init__(self, dir, hf_token=None, device=None, load_in_8bit=True, use_embed_head=False, device_map='auto'):
        if device is None:
